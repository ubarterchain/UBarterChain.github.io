...
- 在3里输入账户开启密码；点击”启用账户“；将开启账户供用于交换操作。
- 启用账户，在上图的3里输入文件名；点击"备份银行”；将备份银行到bin内的指定文件里。
- 启用账户，在3里输入公钥；点击"查询私钥”；将显示该私钥在3里。
- 启用银行，在3里输入私钥；点击"存入私钥”；将指定私钥对应的支票存入账户供交换使用。
...
##### 验证密语过程如下： ##### 
- 在1里输入本节点拥有的支票代码；在2里输入加密用的信息；在3里输入需要验证的密语；在5里输入账户启用密码；点击“启用账户”；点击“加密信息”；将在4里显示出密语验证的结果。
